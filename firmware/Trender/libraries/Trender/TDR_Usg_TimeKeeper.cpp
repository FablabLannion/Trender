...
// class TDR_Usg_TimeKeeper : public TDR_Usage 
TDR_Usg_TimeKeeper::TDR_Usg_TimeKeeper(){
	_ptk       = new Ticker();
	_startTime = 0;
	_stopTime  = 0;
...
TDR_Usg_TimeKeeper::TDR_Usg_TimeKeeper(TDR_NeoPixel* n) {
    _ptk       = new Ticker();
    _startTime = 0;
    _stopTime  = 0;
    _previousMode = STOPPED;
...
/** start timer
 */
int  TDR_Usg_TimeKeeper::start (void) {
    ...
    _ptk->attach(1, tkTDR_Usg_TimeKeeper, this);
...
/** stop main timer
 */
int  TDR_Usg_TimeKeeper::stop (void) {
    _pstrip->setColor(0);
 ...
 */
void TDR_Usg_TimeKeeper::readConfig (void) {
    int i = 0, j = 0;
    uint8_t c;
...
void TDR_Usg_TimeKeeper::setStrip(TDR_NeoPixel* s) {
    _pstrip=s;
}

TDR_NeoPixel* TDR_Usg_TimeKeeper::getStrip() {
	return _pstrip;
}

char* TDR_Usg_TimeKeeper::get_type() {
    return _type;
}
 ...
 */
void writeConfig(TDR_Usg_TimeKeeper *t) {
    if(t==NULL) { Serial.print(__FUNCTION__); Serial.println(": [ERROR] :: TDR_Usg_TimeKeeper should be instancied"); return; }
    int i = 0, j = 0;
...
void sendConfigData (TDR_WebServer* w,TDR_Usg_TimeKeeper *t) {
...
void processConfig (TDR_WebServer* w,TDR_Usg_TimeKeeper *t) {
    if(w==NULL) { Serial.print(__FUNCTION__); Serial.println(": [ERROR] :: TDR_WebServer should be instancied"); return; }
    if(t==NULL) { Serial.print(__FUNCTION__); Serial.println(": [ERROR] :: TDR_Usg_TimeKeeper should be instancied"); return; }
 ...
 */
void tkTDR_Usg_TimeKeeper(TDR_Usg_TimeKeeper* n) {
    ///Serial.println(__FUNCTION__);
    if(n==NULL) { Serial.print(__FUNCTION__); Serial.println(": [ERROR] :: TDR_Usg_TimeKeeper should be instancied"); return; }
