...
// class TDR_TimeKeeper : public TDR_Usage 
...
TDR_TimeKeeper::TDR_TimeKeeper(TDR_NeoPixel* n) {
    _ptk       = new Ticker();
    _startTime = 0;
    _stopTime  = 0;
    _previousMode = STOPPED;
...
int  TDR_TimeKeeper::start (void) {
    ...
    _ptk->attach(1, tkTDR_TimeKeeper, this);
 ...
 */
void TDR_TimeKeeper::readConfig (void) {
    int i = 0, j = 0;
    uint8_t c;
...
void TDR_TimeKeeper::setStrip(TDR_NeoPixel* s) {
    _pstrip=s;
}

TDR_NeoPixel* TDR_TimeKeeper::getStrip() {
	return _pstrip;
}

char* TDR_TimeKeeper::get_type() {
    return _type;
}
 ...
 */
void writeConfig(TDR_TimeKeeper *t) {
    if(t==NULL) { Serial.print(__FUNCTION__); Serial.println(": [ERROR] :: TDR_TimeKeeper should be instancied"); return; }
    int i = 0, j = 0;
...
void sendConfigData (TDR_WebServer* w,TDR_TimeKeeper *t) {
...
void processConfig (TDR_WebServer* w,TDR_TimeKeeper *t) {
    if(w==NULL) { Serial.print(__FUNCTION__); Serial.println(": [ERROR] :: TDR_WebServer should be instancied"); return; }
    if(t==NULL) { Serial.print(__FUNCTION__); Serial.println(": [ERROR] :: TDR_TimeKeeper should be instancied"); return; }
 ...
 */
void tkTDR_TimeKeeper(TDR_TimeKeeper* n) {
    Serial.println(__FUNCTION__);
    if(n==NULL) { Serial.print(__FUNCTION__); Serial.println(": [ERROR] :: TDR_TimeKeeper should be instancied"); return; }
